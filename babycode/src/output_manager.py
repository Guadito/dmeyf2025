...
#----------------------------> Guardar predicciones finales pandas
...
def guardar_predicciones_finales(resultados_df: pd.DataFrame, nombre_archivo=None) -> str:
    ...
    Args:
        resultados_df: DataFrame de pandas con 'numero_de_cliente' y 'Predict'
    ...
    if not isinstance(resultados_df, pd.DataFrame):
        raise TypeError("resultados_df debe ser un DataFrame de pandas")
    ...
    if not np.issubdtype(resultados_df['numero_de_cliente'].dtype, np.integer):
    ...
    if not np.issubdtype(resultados_df['Predict'].dtype, np.integer):
...
def guardar_predicciones_finales_polars(resultados_df: pl.DataFrame, nombre_archivo=None) -> str:
    """
    Guarda las predicciones finales en un archivo CSV en la carpeta predict.
